...
class HOAuthAction extends CAction
{
	...
	protected function prepareGuestUser()
	{
		...
		if($this->oAuth->isBond) {
			...
			if($this->useYiiUser) {
				$user = $this->userModel->findByAttributes(array('username' => $userProfile->emailVerified));
			} else {
	...
	protected function processUser($user, $userProfile)
	{
		if($this->useYiiUser) {
			...
			$prop = new ReflectionProperty('TblUsersProfiles', 'regMode');
			if($prop->isStatic())
				TblUsersProfiles::$regMode = true;
		}
	...
	protected function sendActivationEmail($user)
	{
		if($this->useYiiUser) {
			...
			if (Yii::app()->getModule('user')->sendActivationMail) {
				$activation_url = Yii::app()->createAbsoluteUrl('/user/activation/activation',array("activkey" => $user->activkey, "email" => $user->username));
				SiteHelper::sendMail($user->username,UserModule::t("You registered on {site_name}",array('{site_name}'=>Yii::app()->name)),UserModule::t("To activate your account, please go to {activation_url}",array('{activation_url}'=>$activation_url)));
			}
		} else {
