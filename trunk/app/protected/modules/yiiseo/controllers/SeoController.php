...
//                'expression'=>'isset(Yii::app()->userseo->name)',
	...
	public function actionCreate() {
            ...
            if (isset($_POST['YiiseoUrl'])) {
                $model->attributes = Yii::app()->request->getParam('YiiseoUrl');
                /* save url */
                if ($model->save()) {
                    /* save MetaName */
                    if (isset($_POST['YiiseoMain'])) {
                        $items = Yii::app()->request->getParam('YiiseoMain');
                        foreach ($items as $name => $item) {
                            $mod = new YiiseoMain();
                            $mod->attributes = $item;
                            $mod->name = $name;
                            $mod->url = $model->id;
                            if(empty($mod->active) && !empty($mod->content)) {
                                $mod->active = 1;
                            }
                            $mod->save();
                        }
                    ...
                    if (isset($_POST['YiiseoProperty'])) {
                        $properties = Yii::app()->request->getParam('YiiseoProperty');
            ...
            $this->render('create', array(
                ...
                'listParams' => $this->getParams(!empty($model->url) ? $model->url : ''),
            ));
        }
    ...
    */
    public function actionAddmetaname(){
        ...
        $listParams = $this->getParams(Yii::app()->request->getParam('url'));
        $this->renderPartial("_formMetaName",array('model'=>$model, 'listParams'=>$listParams));
    }
    ...
    */
    public function actionAddmetaproperty(){
        ...
        $listParams = $this->getParams(Yii::app()->request->getParam('url'));
        $this->renderPartial("_formMetaProperty",array('model'=>$model,'count'=>Yii::app()->request->getParam('count'), 'listParams'=>$listParams));
    }
    ...
    */
    ...
    /**
     * Listing for controler
     */
    public function getModels($url)
    {
        $models = array();
        if(empty(Yii::app()->params['seoControllerList'])) {
            return $models;
        }
        
        $controllerList = isset(Yii::app()->params['seoControllerList'])? Yii::app()->params['seoControllerList'] : array();
        $urlArr = explode('/', $url);
        $url = $urlArr[0] . (!empty($urlArr[1])?('/' . $urlArr[1]):'');
        if(!empty($controllerList[$url])) {
            $models = $controllerList[$url];
        }
...
//            {
//                $ext = explode(".",$file);
//                if($ext[1] == "php" && in_array($ext[0], isset(Yii::app()->params['seoModelList'])? Yii::app()->params['seoModelList'] : array()))
//                {
//                    $models[] = $ext[0];
//                }
//            }
//        }
//Yii::log(print_r($models,1),'ERROR');
        return $models;
    }

    /**
     * Listing all params for model
     */
