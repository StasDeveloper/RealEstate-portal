...
class S3AssetManager extends CAssetManager
{
    ...
    /**
     * Method sends and compressed file to S3.
     *
     * @param $path
     * @param bool|false $hashByName
     * @param int $level
     * @param bool|false $forceCopy
     * @return mixed
     * @throws CException
     */
    public function gzipPublish($path, $string, $contentType, $hashByName=false, $level=-1, $forceCopy=false)
    {
        if (isset($this->_published[$path]))
            return $this->_published[$path];
        else if (($src = realpath($path)) !== false)
        {
            ...
            $fileName = basename($src);
            $dstDir = $this->getBasePath().'/'.$dir;
            $dstFile = /*'test2/'.*/ $dstDir.'/'.$fileName. '.gz';
            if ($this->getCache()->get($this->getCacheKey($path)) === false)
            {
                if ($this->getS3()->putObjectString($string, $this->bucket, $dstFile, $acl = S3::ACL_PUBLIC_READ, array(), array('Content-Type'=>$contentType,'Content-Encoding'=>'gzip', 'Cache-Control: max-age = 2592000')))
                {
                    $this->getCache()->set($this->getCacheKey($path), true, 0, new CFileCacheDependency($src));
//                    Yii::log('TRUE we sent file', 'ERROR');
                }
                else
                {
                    Yii::log('FALSE didn\'t sent file', 'ERROR');
                    throw new CException('Could not send asset to S3');
                }
            }

            $this->_published[$path] = $this->getBaseUrl()."/$dir/$fileName".'.gz';

//            Yii::log('Path to CSS: '. $this->_published[$path], 'ERROR');
            return $this->_published[$path];
        }
        throw new CException(Yii::t('yii', 'The asset "{asset}" to be published does not exist.', array('{asset}' => $path)));
    }
