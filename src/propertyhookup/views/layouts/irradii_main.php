...
<!--        --><?php //$cs->registerCssFile('//maxcdn.bootstrapcdn.com/bootstrap/3.0.3/css/bootstrap.min.css'); ?>
<!--        --><?php //$cs->registerCssFile('//maxcdn.bootstrapcdn.com/font-awesome/4.4.0/css/font-awesome.min.css'); ?>

        <!-- SmartAdmin Styles : Please note (smartadmin-production.css) was created using LESS variables -->
        <!--    Old way    -->
<!--        --><?php //$cs->registerCssFile(CPathCDN::baseurl( 'img' ) . '/css/smartadmin-production.css'); ?>
<!--        --><?php //$cs->registerCssFile(CPathCDN::baseurl( 'img' ) . '/css/smartadmin-skins.css'); ?>
<!--        --><?php //$cs->registerCssFile(CPathCDN::baseurl( 'img' ) . '/css/demo.css'); ?>
<!--        --><?php //$cs->registerCssFile(CPathCDN::publish(Yii::app()->theme->basePath . '/css/styles-smartadmin.css', 'text/css')); ?>
<!--        --><?php //$cs->registerCssFile(CPathCDN::publish(Yii::app()->theme->basePath . '/css/styles-smartadmin_1.css', 'text/css')); ?>

        <!--    New way    -->
<!--        --><?php //$cs->registerCssFile(CPathCDN::gzipPublish(Yii::app()->theme->basePath . '/css/concat-style.min.css', 'text/css')); //included .css files are listed in Gruntfile.js ?>
        ...
        <!-- GOOGLE FONT -->
        <?php $cs->registerCssFile('//fonts.googleapis.com/css?family=Open+Sans:400italic,700italic,300,400,700'); ?>
<!--        --><?php //$cs->registerCssFile(CPathCDN::gzipPublish('//fonts.googleapis.com/css?family=Open+Sans:400italic,700italic,300,400,700|Open+Sans+Condensed:300|Muli', 'text/css')); ?>
        <?php $cs->registerCssFile('http://devirradii.com/themes/propertyhookup/TEST/css/concat-style.min.css'); ?>

    </head>
    <!--here-->
    <body <?php if (isset($this->body_ID)) {
            echo $this->body_ID;
        }
        if (isset($this->body_onload)) {
            echo $this->body_onload;
        }
        ?> class="minified">
                ...
                <div class="header-right-container">
                    ...
                    <?php if (!Yii::app()->user->isGuest): ?>
                        <div id="logout" class="btn-header transparent pull-right">
                            <span> <a href="<?php echo  Yii::app()->createUrl('/user/logout'); ?>" title="Sign Out"><i class="fa fa-sign-out"></i></a> </span>
                        </div>
                    <?php endif; ?>
    ...
    <?php else: ?>
                    ...
                    <div class="header-right-container">
                        ...
                        <div id="login-block" class="btn-header transparent pull-right">
                            <span> <a href="<?php echo Yii::app()->createUrl('user/login') ?>" title="Log In"><i class="fa fa-sign-in"></i><span class="font-sm">&nbsp;Sign In&nbsp;</span></a> </span>
                        </div>
                        ...
                        <div id="create-account-block" class="btn-header transparent pull-right">
                            <span> <a href="<?php echo Yii::app()->createUrl('user/registration') ?>" title="Log In"><i class="fa fa-plus"></i><span class="font-sm">&nbsp;Join&nbsp;</span></a> </span>
                        </div>								
...
<!--        --><?php //$cs->registerScriptFile('//underscorejs.org/underscore-min.js', CClientScript::POS_END); ?>


<!--        --><?php //$cs->registerCoreScript('jquery');

$cs->registerScript("addJqueryOne", "
    var jqueryOne = jQuery;
        ...
        <!-- BOOTSTRAP JS -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/bootstrap/bootstrap.min.js', CClientScript::POS_END); ?>


        <!-- CUSTOM NOTIFICATION -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/notification/SmartNotification.min.js', CClientScript::POS_END); ?>


        <!-- JARVIS WIDGETS -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/smartwidgets/jarvis.widget.min.js', CClientScript::POS_END); ?>


        <!-- EASY PIE CHARTS -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/easy-pie-chart/jquery.easy-pie-chart.min.js', CClientScript::POS_END); ?>


        <!-- SPARKLINES -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/sparkline/jquery.sparkline.min.js', CClientScript::POS_END); ?>


        <!-- JQUERY VALIDATE -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/jquery-validate/jquery.validate.min.js', CClientScript::POS_END); ?>


        <!-- JQUERY MASKED INPUT -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/masked-input/jquery.maskedinput.min.js', CClientScript::POS_END); ?>


        <!-- JQUERY SELECT2 INPUT -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/select2/400/select2_custom.js', CClientScript::POS_END); ?>
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/select2/select2.min.js', CClientScript::POS_END); ?>


        <!-- JQUERY UI + Bootstrap Slider -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/bootstrap-slider/bootstrap-slider.min.js', CClientScript::POS_END); ?>


        <!-- browser msie issue fix -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/msie-fix/jquery.mb.browser.min.js', CClientScript::POS_END); ?>


        <!-- SmartClick: For mobile devices -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/smartclick/smartclick.js', CClientScript::POS_END); ?>

        <!-- JQuery Form: For form managing -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/jquery-form/jquery-form.min.js', CClientScript::POS_END); ?>
        ...
        <![endif]-->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/demo.js', CClientScript::POS_END); ?>

        <!-- MAIN APP JS FILE -->
        <?php // $cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/app.js', CClientScript::POS_END); ?>

        <!-- PAGE RELATED PLUGIN(S) -->

        <!-- Flot Chart Plugin: Flot Engine, Flot Resizer, Flot Tooltip -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/flot/jquery.flot.cust.js', CClientScript::POS_END); ?>
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/flot/jquery.flot.resize.js', CClientScript::POS_END); ?>
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/flot/jquery.flot.tooltip.js', CClientScript::POS_END); ?>

        <!-- Vector Maps Plugin: Vectormap engine, Vectormap language -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/vectormap/jquery-jvectormap-1.2.2.min.js', CClientScript::POS_END); ?>
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/vectormap/jquery-jvectormap-world-mill-en.js', CClientScript::POS_END); ?>

        <!-- Full Calendar -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . '/js/plugin/fullcalendar/jquery.fullcalendar.min.js', CClientScript::POS_END); ?>
        <!-- PACE LOADER - turn this on if you want ajax loading to show (caution: uses lots of memory on iDevices)-->
        ...
        <!-- PAGE RELATED PLUGIN(S) -->
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . "/js/plugin/bootstrap-progressbar/bootstrap-progressbar.js", CClientScript::POS_END); ?>
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . "/js/plugin/datatables/jquery.dataTables-cust.min.js", CClientScript::POS_END); ?>
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . "/js/plugin/bootstrap-tags/bootstrap-tagsinput.min.js", CClientScript::POS_END); ?>
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . "/js/plugin/datatables/ColReorder.min.js", CClientScript::POS_END); ?>

        <?php // $cs->registerScriptFile(Yii::app()->theme->getBaseUrl() . '/js/concat-build.min.js', CClientScript::POS_END); // load file from project folder ?>
<!--        --><?php //$cs->registerScriptFile(CPathCDN::gzipPublish(Yii::app()->theme->basePath . '/js/concat-build.min.js', 'text/javascript'), CClientScript::POS_END); //included .js files are listed in Gruntfile.js ?>
        <?php $cs->registerScriptFile('http://devirradii.com/themes/propertyhookup/TEST/js/concat-build.min.js',CClientScript::POS_END); ?>
        <?php $cs->registerScriptFile('//maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&language=en-us&libraries=drawing,places', CClientScript::POS_END); ?>
        <?php $cs->registerScriptFile("//cdn.datatables.net/plug-ins/be7019ee387/sorting/num-html.js", CClientScript::POS_END); ?>
        <?php $cs->registerScriptFile("//cdn.datatables.net/plug-ins/be7019ee387/sorting/currency.js", CClientScript::POS_END); ?>
        <?php $cs->registerScriptFile("//cdn.datatables.net/plug-ins/be7019ee387/sorting/natural.js", CClientScript::POS_END); ?>


        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . "/js/plugin/datatables/FixedColumns.min.js", CClientScript::POS_END); ?>
        <?php // do not uncomment $cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . "/js/plugin/datatables/ColVis.min.js", CClientScript::POS_END); ?>
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . "/js/plugin/datatables/ColVisNew.js", CClientScript::POS_END); ?>
        <?php // do not uncomment $cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . "/js/plugin/datatables/ColVis.js", CClientScript::POS_END); ?>
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . "/js/plugin/datatables/ZeroClipboard.js", CClientScript::POS_END); ?>
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . "/js/plugin/datatables/media/js/TableTools.min.js", CClientScript::POS_END); ?>
        <?php //$cs->registerScriptFile(CPathCDN::baseurl( 'js' ) . "/js/plugin/datatables/DT_bootstrap.js", CClientScript::POS_END); ?>


        <?php $cs->registerScript("setUserOnline", "
...
//               '#details_full_access, #details_analyze,'+
        ...
        $cs->registerScript("clickLogo", " 
            $('#logo').click(function(e){
                e.preventDefault();
                window.location.href = '" . Yii::app()->request->getBaseUrl(true) . "';
            });
                 ", CClientScript::POS_READY);

$cs->registerScript("ganalitics","
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button data-dismiss="modal" class="close closeModal" type="button">×</button><h1><?php echo UserModule::t("Attention!"); ?></h1>
                    </div>
