...
//                        console.log('error: ' + xhr.responseCode);
                ...
                for(var i = 0; i < latLon_arr.length; i++){
                    for(var key in latLon_arr[i]){
                        ...
                        marker = new google.maps.Marker({
                                ...
                                map: map,
                                icon: image,
                                property_id: latLon_arr[i][key].property_id,
            ...
            function showDetailPopup(marker) {
                var prop_id = marker.property_id,
                ...
                descArray.forEach(function(e){
                    if(e && e != ''){
                       if(e.indexOf('Price') >= 0){
                            ...
                            descObject['description'] = descObject['description'] + ' ' + e;
                       }
                    }
                });

                street = row$.find('.property_info_row_map').data('address');
                ...
                $('.detail-pop-up')
                    ...
                    .find('.label-container')
                        .append(descObject['label'])
                    .end()
                    .find('.price').html(descObject['price'])
                    .end()
                    .find('.street').html(street)
                    .end()
                    .fadeIn()
                    .appendTo('#map_canvas');
                $.ajax({
